...
def ordered_products_per_customer() -> str:
    ...
    res = []
    for o in orders:
        res.append(f"Order ID: {o.id}, Customer: {o.customer.username}")
        for op in o.orderproduct_set.all():
            res.append(f"- Product: {op.product.name}, Category: {op.product.category.name}")

    return '\n'.join(res)

def filter_products() -> str:
    products = Product.objects.available_products().filter(price__gt=3.00).order_by('-price', 'name')
